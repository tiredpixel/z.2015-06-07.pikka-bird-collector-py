...
class TestRedis:
    ...
    def mock_error(self):
        f = os.path.join(os.path.dirname(__file__), '../fixtures/redis/error.txt')
        return TestRedis.read_fixture(f)
    
    def mock_info(self):
        f = os.path.join(os.path.dirname(__file__), '../fixtures/redis/info.txt')
        return TestRedis.read_fixture(f)
    ...
    def test_parse_output_info(self):
        assert Redis.parse_output(self.mock_info()) == {
            'clients': {
                'blocked_clients': 0,
    ...
    def test_parse_output_cluster_info(self):
        assert Redis.parse_output(self.mock_cluster_info()) == {
            'cluster_my_epoch': 2,
