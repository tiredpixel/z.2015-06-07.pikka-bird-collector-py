...
class System(Base):
    ...
    def __memory(self):
        ...
        metrics_virtual = {
            ...
            'virtual_active_/':
                'active' in virtual_fs   and round(virtual.active / virtual.total, self.RATIO_DP),
            'virtual_inactive_b':
                'inactive' in virtual_fs and virtual.inactive,
            'virtual_inactive_/':
                'inactive' in virtual_fs and round(virtual.inactive / virtual.total, self.RATIO_DP),
            'virtual_buffers_b':
                'buffers' in virtual_fs  and virtual.buffers,
            'virtual_buffers_/':
                'buffers' in virtual_fs  and round(virtual.buffers / virtual.total, self.RATIO_DP),
            'virtual_cached_b':
                'cached' in virtual_fs   and virtual.cached,
            'virtual_cached_/':
                'cached' in virtual_fs   and round(virtual.cached / virtual.total, self.RATIO_DP),
            'virtual_wired_b':
                'wired' in virtual_fs    and virtual.wired,
            'virtual_wired_/':
                'wired' in virtual_fs    and round(virtual.wired / virtual.total, self.RATIO_DP),
            'virtual_shared_b':
                'shared' in virtual_fs   and virtual.shared,
            'virtual_shared_/':
                'shared' in virtual_fs   and round(virtual.shared / virtual.total, self.RATIO_DP)}
